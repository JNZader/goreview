...
describe('Repository Configuration', () => {
    ...
    it('returns defaults when no config file', async () => {
        ...
        const config = await loadRepoConfig(mockOctokit, 'owner', 'repo');
...
`;
    ...
    it('caches configuration', async () => {
        ...
        await loadRepoConfig(mockOctokit, 'owner', 'repo');
        await loadRepoConfig(mockOctokit, 'owner', 'repo');
...
`;
    ...
    it('handles different refs', async () => {
        ...
        await loadRepoConfig(mockOctokit, 'owner', 'repo', 'main');
        await loadRepoConfig(mockOctokit, 'owner', 'repo', 'develop');
        // Should call API twice for different refs
        expect(mockOctokit.repos.getContent).toHaveBeenCalledTimes(2);
    });
});
//# sourceMappingURL=repoConfig.test.js.map