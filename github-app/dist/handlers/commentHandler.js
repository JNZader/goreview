 ...
 */
export async function handleIssueComment(event) {
 ...
 */
async function generateResponse(command, context) {
 ...
 * Build prompt for AI
 */
function buildPrompt(command, context) {
    const baseContext = `
 ...
 * Format AI response
 */
function formatResponse(aiResponse, commandType) {
    // Add a header
 ...
 */
function getResponseHeader(commandType) {
    switch (commandType) {
        case 'explain':
            return '### Explanation';
        case 'suggest':
 ...
 */
function generateFallbackResponse(commandType) {
    switch (commandType) {
        case 'explain':
