...
describe('handleInstallation', () => {
    ...
    const createPayload = (action, installationId = 12345, accountLogin = 'testuser', accountType = 'User', repositories) => ({
        action,
        installation: {
            id: installationId,
    ...
    it('should log debug for unhandled installation actions', async () => {
        ...
        expect(logger.debug).toHaveBeenCalledWith({ action: 'suspend' }, 'Unhandled installation action');
    });
...
//# sourceMappingURL=installationHandler.test.js.map