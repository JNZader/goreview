...
describe('handlePullRequest', () => {
    beforeEach(() => {
        ...
        loadRepoConfig.mockResolvedValue({
            review: {
                auto_review: true,
                max_files: 50,
            },
    ...
    it('should process PR synchronize event', async () => {
        ...
        expect(logger.info).toHaveBeenCalledWith(expect.objectContaining({ action: 'synchronize' }), 'Processing pull request');
    });
    ...
    it('should ignore irrelevant PR actions', async () => {
        ...
        expect(logger.debug).toHaveBeenCalledWith({ action: 'closed' }, 'Ignoring PR action');
        expect(jobQueue.add).not.toHaveBeenCalled();
    });
    it('should skip draft PRs', async () => {
        const payload = createPayload({ draft: true });
        await handlePullRequest('opened', payload);
        expect(logger.info).toHaveBeenCalledWith({ pullNumber: 42 }, 'Skipping draft PR');
        expect(jobQueue.add).not.toHaveBeenCalled();
    });
    it('should skip when auto-review is disabled', async () => {
        ...
        await handlePullRequest('opened', payload);
        expect(logger.info).toHaveBeenCalledWith({ owner: 'owner', repo: 'repo' }, 'Auto-review disabled for repository');
        expect(jobQueue.add).not.toHaveBeenCalled();
    });
    ...
    it('should log when job is queued', async () => {
        const payload = createPayload();
        await handlePullRequest('opened', payload);
        expect(logger.info).toHaveBeenCalledWith({ owner: 'owner', repo: 'repo', pullNumber: 42 }, 'PR review job queued');
    });
});
//# sourceMappingURL=pullRequestHandler.test.js.map