...
function consumeToken(bucket, config) {
...
export function createRateLimiter(config = {}) {
    ...
    return (req, res, next) => {
        ...
        const resetTime = Math.ceil((fullConfig.tokensPerRequest - result.bucket.tokens) / fullConfig.refillRate * 1000);
 ...
 */
export function recordFailedAttempt(ip, maxAttempts = 5, windowMs = 900000) {
    const now = Date.now();
 ...
 */
export function getRateLimitInfo(key, config = RateLimitPresets.standard) {
    ...
    const resetTime = Math.ceil((config.tokensPerRequest - refilled.tokens) / config.refillRate * 1000);
    return {
        limit: config.maxTokens,
        remaining,
 ...
 */
export function getTrackedClientCount() {
