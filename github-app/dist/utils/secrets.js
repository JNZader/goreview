 ...
 */
export function encryptSecret(plaintext, masterPassword) {
 ...
 */
export function decryptSecret(encryptedData, masterPassword) {
    const password = masterPassword ?? getMasterKey();
 ...
 */
export function maskSecret(secret, options = {}) {
    const { showFirst = 4, showLast = 4, maskChar = '*', minLength = 12, } = options;
 ...
 */
export function safeStringify(obj, sensitiveKeys = ['password', 'secret', 'token', 'key', 'apiKey', 'privateKey', 'authorization']) {
 ...
 */
export function createHmac(value, secret) {
